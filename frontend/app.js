...
document.getElementById("loginForm")?.addEventListener("submit", async (e) => {
  ...
  const res = await fetch(
    `${API_BASE}/doctors/by_email?email=${encodeURIComponent(email)}`
  );
...
async function loadDashboard() {
  ...
  data.couples.forEach((c) => {
    ...
    row.innerHTML = `
      ...
      <td>
        <a href="timeline.html?couple_id=${
          c.couple_id
...
document.getElementById("answerForm")?.addEventListener("submit", async (e) => {
  ...
  const res = await fetch(
    `${API_BASE}/assessments/${assessmentId}/answers/bulk`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
document
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    const res = await fetch(`${API_BASE}/assessments/${id}/predict`, {
      method: "POST",
    });
...
// Fetch couple details to personalize labels
async function loadCoupleNames() {
  const coupleId = localStorage.getItem("currentCoupleId");
  if (!coupleId) return;

  const res = await fetch(`${API_BASE}/couples/${coupleId}`);
  if (!res.ok) return;

  const couple = await res.json();

  // Save names for later use
  localStorage.setItem("partnerAName", couple.partner_a_name || "Partner A");
  localStorage.setItem("partnerBName", couple.partner_b_name || "Partner B");

  document.getElementById(
    "labelAnswerA"
  ).textContent = `${couple.partner_a_name} Answer (0–4):`;
  document.getElementById(
    "labelAnswerB"
  ).textContent = `${couple.partner_b_name} Answer (0–4):`;
}
